...
class _PlayerItemState extends State<PlayerItem>
  ...
  Future<void> handleShortcutForwardRepeat() async {
    final double defaultShortcutForwardPlaySpeed = setting.get(
    ...
    try {
      ...
      unawaited(
        _audioController.updateSession(
          ...
          title: bangumiTitle,
          album: videoPageController.sourceName,
          artist: episodeTitle,
          artUri: artworkUri,
  ...
  Future<void> handleSuperResolutionChange(int shaderIndex) async {
    ...
    final bool alreadyShown = setting.get(
  ...
  Timer getPlayerTimer() {
    return Timer.periodic(const Duration(seconds: 1), (timer) {
      ...
      if (playerController.playback.playerPlaying &&
        ...
        final pluginName = videoPageController.sourceName;
        final selection = videoPageController.playbackEpisode;
  ...
  void showSyncPlayRoomCreateDialog() {
    ...
    String room = '';
    String username = '';
    final isSwitchingRoom = playerController.syncplay.syncplayRoom != '';
    KazumiDialog.show(
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text(isSwitchingRoom ? '切换房间' : '加入房间'),
          content: Form(
            key: formKey,
            child: Column(
              mainAxisSize: MainAxisSize.min,
          ...
          actions: [
            ...
            TextButton(
              ...
              child: Text(isSwitchingRoom ? '切换' : '确定'),
            ),
          ],
        );
      },
  ...
  void initState() {
    ...
    backgroundPlayback = setting.get(
      SettingBoxKey.backgroundPlayback,
      defaultValue: false,
    );
    brightnessVolumeGesture = setting.get(
  ...
  Widget build(BuildContext context) {
    collectType = collectController.getCollectType(
      videoPageController.bangumiItem,
    );
    return Observer(
      builder: (context) {
                        ...
                        child: Focus(
                          ...
                          onKeyEvent: (focusNode, KeyEvent event) {
                            ...
                            if (event is KeyDownEvent) {
                              ...
                              handled = handleShortcutLongPress(
                                keyLabel,
                                "Repeat",
                              );
